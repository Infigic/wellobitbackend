    ...
    public function index()
    {
        ...
        if ($frequency === 'daily') {
            ...
            return $this->sendResponse([
                ...
                'hrv' => $this->transformHrvRecords($hrvs, $baseline_value),
                'average' => [
                    'stressed' => $stressedPercentage,
                    'balanced' => $balancedPercentage,
                    'happy' => $happyPercentage,
            ...
            while ($current->lte($endDate)) {
                ...
                $stressedSum = $dayRecords->where('status', 3)->sum('hrv');
                $balancedSum = $dayRecords->where('status', 2)->sum('hrv');
                $happySum = $dayRecords->where('status', 1)->sum('hrv');
