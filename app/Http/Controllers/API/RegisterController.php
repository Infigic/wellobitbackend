...
//      dd(now());
  ...
  public function resendOtp(Request $request) {
    ...
    if ($user->otp && $user->otp_expires_at && Carbon::now()->lt($user->otp_expires_at)) {
      ...
      return $this->sendResponse(['otp' => $otp,], 'OTP resent successfully (existing OTP still valid).');
    }
    ...
    return $this->sendResponse(['otp' => $otp], 'A new OTP has been sent to your email.');
  }
  ...
  protected function decodeIdToken(string $idToken, string $platform): object {
    try {
      ...
      return JWT::decode(
          $idToken, JWK::parseKeySet($keys, 'RS256')
      );
    } catch (\Throwable $e) {
  ...
  public function forgetPassword(Request $request) {
    ...
    $this->sendOtpEmail($user->email, $otp, $user->name);
    return $this->sendResponse(['id' => $user->id, 'otp' => $otp], 'Password reset OTP sent successfully. Please check your email.');
  }
