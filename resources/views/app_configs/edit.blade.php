                    ...
                    <form action="{{ route('app-configs.update', $appConfig->id) }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('config_key') is-invalid @enderror"
                                ...
                                value="{{ old('config_key', $appConfig->config_key ?? '') }}" placeholder="Enter config key"
                                required>
                        ...
                        <div class="form-group">
                            ...
                            <select name="value_type" class="form-select" id="value_type" required>
                                <option value="string"
                                    {{ old('value_type', $appConfig->value_type ?? 'string') == 'string' ? 'selected' : '' }}>
                                    String</option>
                                <option value="boolean"
                                    {{ old('value_type', $appConfig->value_type ?? 'boolean') == 'boolean' ? 'selected' : '' }}>
                                    Boolean</option>
                                <option value="integer"
                                    {{ old('value_type', $appConfig->value_type ?? 'integer') == 'integer' ? 'selected' : '' }}>
                                    Integer</option>
                                <option value="json"
                                    {{ old('value_type', $appConfig->value_type ?? 'json') === 'json' ? 'selected' : '' }}>
                                    JSON</option>
                            </select>
                        </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const oldType = @json(old('value_type', $appConfig->value_type ?? 'string'));
            const oldConfigValue = @json(old('config_value', $appConfig->config_value ?? ''));
